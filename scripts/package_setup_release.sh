...
#
# Writes: dist/resident-evil-3-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix resident-evil-3 \
  --exe-name Resident_Evil_3__Nemesis_Recompiled \
  --display-name "Resident Evil 3: Nemesis Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --runtime-dir mods \
  --disc-hint "Resident Evil 3 - Nemesis (Europe).cue" \
